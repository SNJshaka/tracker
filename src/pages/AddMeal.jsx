...
const AddMeal = () => {
  ...
  const macroTotal = (macro) =>
    (Number(macro?.slice(0, -1)) || 0) * mealInfo.numOfServings;

  // const macroPercentageTotal = macroPercentage => macroPercentage * mealInfo.numOfServings

  const { fetchedMeals, fetchedMeal } = useSelector(
    (store) => store.fetchedMealsSlice
  );
  const { nutrition, title } = fetchedMeal;
  if (!nutrition) return;

  const { fat, protein, carbs } = nutrition;
  const { percentFat, percentProtein, percentCarbs } =
  ...
  const addMealHandler = () => {
    ...
    dispatch(
      addMeal({ id, ...fetchedMeal, mealImage, mealType, numOfServings })
    );
        ...
        <div className="divide-y-2 p-2">
          ...
          <div className="flex justify-between items-center p-4">
            ...
            <div className="text-center">
              <p className="text-xs 2xl:text-lg 3xl:text-2xl text-yellow-500">
                {percentCarbs.toFixed()}%
              </p>
              <p className="text-sm 2xl:text-lg 3xl:text-2xl">
                {macroTotal(carbs)}
              </p>
              <p className="text-xs 2xl:text-lg 3xl:text-2xl text-gray-400">
                Carbs
              </p>
            </div>
            {/* children */}
            <div className="text-center">
              <p className="text-xs 2xl:text-lg 3xl:text-2xl text-orange-500">
                {percentFat.toFixed()}%
              </p>
              <p className="text-sm 2xl:text-lg 3xl:text-2xl">
                {macroTotal(fat)}
            ...
            {/* children */}
            <div className="text-center">
              <p className="text-xs 2xl:text-lg 3xl:text-2xl text-green-500">
                {percentProtein.toFixed()}%
              </p>
              <p className="text-sm 2xl:text-lg 3xl:text-2xl">
                {macroTotal(protein)}
              </p>
