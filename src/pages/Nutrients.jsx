...
const Nutrients = () => {
  const { recommendedNutrientsIntake } = useSelector(
    (store) => store.userInfoSlice
  );
  const { storedMeals } = useSelector((store) => store.storedMealsSlice);
  const nutrientsList = storedMeals.map((meal) => [
  ...
  const filterNutrients = (nutrientName) => {
    return nutrientsList.map((nutrients) => {
      const filteredNutrients = nutrients[0].find(
  ...
  const nutrientsArray = mainNutrients.map((nutrient) => {
    ...
    const nutrientsTotal = nutrient.reduce(
      (total, item) => total + (item.amount || 0) * item.numOfServings,
      0
    );
    return { name: nutrientsName, total: nutrientsTotal };
  });

  const recommendedNutrientsIntakeArray = Object.entries(
    recommendedNutrientsIntake
  );

  const nutrientPercentage = (nutrientAmount, total) =>
    (nutrientAmount / total) * 100;
  ...
  return (
    <div className="bg-white h-[calc(100vh-190px)] 2xl:h-[calc(100vh-230px)] overflow-y-scroll rounded-md p-4 text-black divide-y-2 ">
      ...
      {nutrientsArray.map((nutrient, index) => {
        const width = nutrientPercentage(
          nutrient?.total,
          recommendedNutrientsIntakeArray[index][1]
        );
        return (
          <div className="py-4" key={index}>
            <div className="grid grid-cols-4 justify-items-center mb-2">
              <p className="justify-self-start">
                {" "}
                {nutrient.name ||
                  recommendedNutrientsIntakeArray[index][0]
                    .slice(0, 1)
                    .toUpperCase() +
