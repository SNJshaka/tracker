...
const Diary = () => {
  ...
  const { totalCalories } = useSelector(
    (store) => store.storedMealsSlice.caloriesEaten
  );

  const outerContainerClasses =
    "text-black text-sm 2xl:text-lg 3xl:text-2xl overflow-y-scroll";
  const caloriesUpperTextClasses = "text-sm 2xl:text-lg 3xl:text-2xl";
  const caloriesBottomTextClasses =
    "text-xs 2xl:text-lg 3xl:text-2xl text-gray-500";
  const seperatorsClasses = "flex flex-col items-center";

  return (
    <div className={outerContainerClasses}>
      {/* header card */}
      <div className="bg-white p-4 mb-4 shadow-md shadow-white/50 rounded-md hover:shadow-sm duration-200 mt-8">
        <h2 className={caloriesUpperTextClasses + " mb-2"}>
          {" "}
          Calories Remaining{" "}
        </h2>
        <div className="flex justify-between items-center">
          ...
          {/* 7th item */}
          <div className={seperatorsClasses}>
            <p className={caloriesUpperTextClasses + " font-bold"}>
              {" "}
              {caloriesRequired - totalCalories}{" "}
            </p>
